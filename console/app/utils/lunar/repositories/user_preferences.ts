...
export class UserPreferencesRepository extends BaseRepository {
  ...
  async get_by_identifier(identifier: string): Promise<UserPreferences> {
    ...
    if (!row) throw new Error(`user preferences with id ${identifier} not found`);
    return row;
  }

  async update(identifier: string, payload: UpdateUserPreferences): Promise<UserPreferences> {
